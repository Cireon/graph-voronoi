...
namespace GraphVoronoi
{
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
        ...
        {
            int i;
            for (i = 0; i < this._baseHeap.Count; i++)
            {
                if (!Equals(this._baseHeap[i].Value, value)) continue;

                if (this._comparer.Compare(this._baseHeap[i].Key, newPriority) == -1)
                    throw new Exception("Can not increase priorities");

                this._baseHeap[i] = new KeyValuePair<TPriority, TValue>(newPriority, value);
                break;
            }
        ...
        private void insert(TPriority priority, TValue value)
        {
            ...
            this.heapifyFromEndToBeginning(_baseHeap.Count - 1);
        }
